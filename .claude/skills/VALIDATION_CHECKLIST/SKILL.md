...
- [ ] **CI/CD File Naming:** The strategy source file MUST be named `{safe_name}_strategy.py` (e.g., `rsi_divergence_strategy.py`). A generic name or a missing `_strategy` suffix is a FAIL — reject and ask the Orchestrator to re-invoke the Transpiler to rename the file.